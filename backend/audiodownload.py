...
f = open("/var/www/youtube-backend/loga.txt", "r")
match = 0
for x in f:
    l = x.split()
    if(l[0] == url):
...
if(match == 0):
    nuuid = uuid.uuid4()
    name = str(nuuid) + ".mp3"
    youout = subprocess.check_output("/usr/local/bin/youtube-dl --quiet --no-mtime --extract-audio --audio-format mp3 --output /var/www/youtube-backend/file/"+ name + " " + url + "; exit 0", shell=True, stderr=subprocess.STDOUT)
    curtime = datetime.now()
    if(path.exists("/var/www/youtube-backend/file/" + name)):
        f = open("/var/www/youtube-backend/loga.txt", "a")
